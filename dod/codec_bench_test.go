...
func BenchmarkUInt64(b *testing.B) {
	b.Run("sequential", func(b *testing.B) {
		...
		b.Run("decode", func(b *testing.B) {
			encoded := EncodeUInt64(nil, src)
			dst := make([]uint64, benchmarkSize)
	...
	b.Run("timestamps", func(b *testing.B) {
		src := make([]uint64, benchmarkSize)
		ts := uint64(1700000000000) // Unix milliseconds
		for i := range src {
			ts += 15000 + uint64(rand.IntN(100)) // ~15s intervals with jitter
			src[i] = ts
		}
